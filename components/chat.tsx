...
import { ChevronRight, ChevronDown, Loader2, PlusCircle, Trash2 } from "lucide-react"
...
export function Chat({ apiKey }: ChatProps) {
  ...
  const { messages, setMessages, input, setInput, handleSubmit, handleInputChange, status } = useChat({
    api: '/api/chat',
    streamProtocol: 'text',
  });
  ...
  const MessageContent = useMemo(() => {
    const MemoizedMessageContent = React.memo(({ message, index }: { message: UIMessage; index: number }) => {
      ...
      const extractThinkingContent = (content: string) => {
        ...
        if (match && match[1]) {
          // Return the content between <think> tags
          return match[1].trim();
        }
        return null;
      };
      ...
      return (
        <>
          ...
          <div className="prose prose-zinc light:prose-invert max-w-none bg-muted/30 rounded-lg px-4 py-3 relative message-transition" data-loaded="true">
            {!isLoading && <CopyButton value={mainContent} src="message" />}
            <ReactMarkdown
              remarkPlugins={[remarkGfm]}
              rehypePlugins={[rehypeRaw, rehypeSanitize]}
              components={renderers}
  ...
  const onSubmit = async (event?: {
    ...
    if (!apiKey.perplexityApiKey) {
      toast({
        title: "API tokens",
        description: "Please configure API tokens in settings first",
        variant: "destructive"
      })
      return
    }
  ...
  const hasApiTokens = apiKey.perplexityApiKey
